...
function TutorDashboard() {
   ...
   useEffect(()=>{
    ...
    axios.get(`${TutorBaseUrl}/tutor/${tutorId}/courseCount`)
    .then((response)=>{
   ...
   console.log(response.data,"data from courses")
    ...
    return (
        <div className="px-4 sm:px-6 md:px-8 pt-4 sm:pt-6 md:pt-8 pb-6 bg-orange-100">
            <ToastContainer/>
          <div className="flex items-center justify-between">
            <h1 className="text-2xl sm:text-3xl font-semibold text-black cursor-pointer">
              Dashboard
            </h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 mt-6">
            ...
            <div className=' h-[10rem] rounded-[8px] bg-white border-l-[4px] border-[#F6C23E] flex items-center justify-between px-[30px] cursor-pointer hover:shadow-lg transform hover:scale-[103%] transition duration-300 ease-out'>
                <div>
                    <h2 className='text-[#1cc88a] text-[11px] leading-[17px] font-bold'>Number of Courses</h2>
    ...
    >
      ...
      <Tooltip />
      <Legend />
      <Line type="monotone" dataKey="total" name="Total courses" stroke="#8884d8" />
    </LineChart>
  </ResponsiveContainer>
            </div>
